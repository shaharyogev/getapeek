...
changeList = (index)=>{
    this.setState({pcListFiles : allRespons[index]}) 
}
...
folderPeek =(index, obj)=>{
    ...
    if(obj ==="file" ){
    this.setState({peek:[{Name:'This is a file',Type:"non"}]})

    }else if(obj ==="emptyFolder" ){
    this.setState({peek:[{Name:'This is a ampty folder',Type:"non"}]})

    }else{
    this.setState({peek:this.state.pcListFiles[index].Dir})
    }    
...
mousePosition = (pageX, pageY)=>{
    try{
        this.setState({mouseLocationX:pageX, mouseLocationY:pageY})
    }catch (error){
        console.log('folderPeek did not work: ' +  error);
    }
}
...
fileNameHandler = (props) => {
    ...
    return (
        ...
        this.setState({peek:[{Name:'This is not a folder',Type:"non"}]})
    )}

    else if (this.fileType(props.Dir, props.Type) === 'emptyFolder'){
    return(
        this.handleHoverLeave(),
        this.setState({pcListFiles:[]}),
        countPath ++
    )}
    ...
    fetch('/pclistdata',{method: 'POST', headers:{'Accept': 'application/json', 'content-Type': 'application/json',},body: JSON.stringify({fileNameData:fileNameData, countPath:countPath, filePathJson:filePathJson,})})
    .then(response => response.json())
    .then(data => indexResponse(data))
