...
function filesInSubFolder(obj, callback){
  ...
  async.eachSeries(obj, function (nextFileInSub, callback){
    dirNext.push({Name:nextFileInSub, Type:path.extname(nextFileInSub)});
    callback();
    ...
    //console.log("All the files in the SUB folder have been processed successfully");
    }
  });
...
function filesInFolder(obj, callback){
  ...
  fs.readdir(obj,function(err,files){
    ...
    async.eachSeries(files, function (file, callback){
      ...
      pcListFiles.push({Name: file, Type: path.extname(file), Dir: dirNext});

    },function(err){
        if(err)
          console.log("async file is not exsist! " + err);
        
        //console.log("All the files in MAIN folder have been processed successfully")
...
router.get('/pclistdata', function (req,res,next){
  try{  
    ...
    filesInFolder("/",function(respond){
      res.json(respond)
      //console.log("The respond was send " + respond)
    });
